    ...
    public function importSheet(Request $request){
        ...
        if($contErros > 0){
            ...
            foreach ($rows as $key => $row) {
                ...
                //if($row[0] == '16527'){ // Trava de teste

                    //Fix: Colunas estão indo com espaço no texto. 
                     ...
                     foreach ($row as $key => $value) {
                      ...
                      if(!empty($row[0])){
                        ...
                        try {
                            ...
                            // Fix : Nome com espaço na frente

                            $nomeProduto =  trim($row[1]);
                        ...
                        if(isset($produtoBC[0])){
                            ...
                            if(!empty($row[45])){ //verifica se existe o cest
                                ...
                                DB::select("UPDATE cest SET mva = '".$row[42]."' WHERE id = '".$row[45]."'");
                            ...
                            if(empty($produtoBC[0]->perfil_contabil_id)){
                                ...
                                $bcPerfilContabil = BCPerfilContabil::create(
                                      [
                                            ...
                                            'ncm_fk_id'                => !empty($row[4])  ? trim($row[4])  : '',
                            ...
                            $ncm = Ncm::where('cod_ncm', trim($row[4]))->first();

                            if(!empty($row[19])){
                                $ncm->dt_inicio_vigencia = Carbon::createFromFormat('d/m/Y',$row[19])->format('Y-m-d');
                            }                                                                                                                               
                            ...
                            try{
                                ...
                                try{
                                 ...
                                 $bcp->ncm_fk_id = trim($row[4]);
                                 if(!empty($row[45])){
                                    $bcp->cest_fk_id = $row[45];
                                 }
                                ...
                                if(!is_object($bcpNcm)){
                                    ...
                                    $bcpNcm->inicio = date('Y-m-d');
                                    $bcpNcm->ncm_fk_id = $row[4];
                                    $bcpNcm->bc_produto_fk_id = $produtoBC->id;
                                    $bcpNcm->save();
                                }                       
                            ...
                            if(is_object($bcPerfilContabilIcms)){
                                ...
                                // Fix : Não estava atualizando a aliquota de ICMS
                                if(isset($row[17])){
                                ...
                                if(!empty($row[42]) && !empty($row[45])){
                                    $bcPerfilContabilIcms->base_legal_st = $row[47];
                                    $bcPerfilContabilIcms->save();
                                }

                                // Verifica se a data inicio está presente para o ICMS
                                if(!empty($row[48])){
                                    $bcPerfilContabilIcms->inicio = Carbon::createFromFormat('d/m/Y',$row[48])->format('Y-m-d');
                                }else{
                                ...
                                if(!empty($row[49])){
                                    $bcPerfilContabilIcms->fim = Carbon::createFromFormat('d/m/Y',$row[49])->format('Y-m-d');
                                }
                            ...
                            try{
                                ...
                                try{
                                    ...
                                    if(!empty($row[45])){
                                        ...
                                        $nome_produto = str_replace(',', '', $row[1]);
                            ...
                            $perfilContabil = BCPerfilContabil::where('ncm_fk_id',trim($row[4]))->first();
                                ...
                                $bcPerfilContabilPis->save();
                                    ...
                                    if(!empty($row[63])){
                                        ...
                                        /*BCPerfilContabilPis::create([
                                         ...
                                         DB::select("INSERT INTO public.bc_perfilcontabil_pis(
                                                    aliquota, cst, base_legal, inicio, bc_perfil_contabil_fk_id)
                                                    VALUES (0.0, '".$row[64]."', '".$row[65]."', '".$bcPerfilContabilPisinicio."', ".$perfil_contabil_id.")");
                                    }
                            ...
                            if(is_object($bcPerfilContabilCofins)){
                                ...
                                $bcPerfilContabilCofins->save();
                                    ...
                                    if(!empty($row[68])){
                                        $base_legal = str_replace(',', '', $row[70]);
                                         DB::select("INSERT INTO public.bc_perfilcontabil_cofins(
                                                    aliquota, cst, base_legal, inicio, bc_perfil_contabil_fk_id)
                                                    VALUES (".$row[68].", '".$row[69]."', '".$base_legal."', '".$bcPerfilContabilCofinsinicio."', ".$perfil_contabil_id.")");
                                    }
                        ...
                        // Atualiza o N do produto no LOTE
                        ...
                        if(is_object($produtoLoteX)){                        
                            ...
                            if(!empty($row[42])){
                                $produtoLoteX->mva = $row[42];
                            }

                            if(!empty($row[45])){
                                $produtoLoteX->cest = $row[45];
                            }    
